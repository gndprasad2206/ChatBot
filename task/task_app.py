...
Based on the following extracted job description entities
, generate questions to fill in any gaps or missing information:

Entities:
{entities}

make sure if any of the things mentioned below are missing in entities you should definetly ask questions 
about it .
1. Job Title
2. Responsibilities
3. Required Skills
...
def extract_entities(job_desc):
    try:
        ...
        print("this is the response I am able to extract ",response)
        response = response.strip().strip("```json").strip("```JSON").strip()
...
def generate_refined_job_description(job_desc, extracted_entities, answers):
    try:
        ...
        refined_response = refined_response.strip("```json").strip("```").strip()
...
if submit_final:
    try:
        answers = {f"answer_{i}": st.session_state.answers[f"answer_{i}"] for i in range(len(st.session_state.questions))}
        refined_job_desc = generate_refined_job_description(st.session_state.job_desc, st.session_state.extracted_entities, answers)
        
        st.subheader("Refined Job Description (JSON):")
        st.json(refined_job_desc)
